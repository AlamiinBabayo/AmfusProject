...
/* ✅ DESKTOP LOGIN: dropdown behaves like About, but blue button */
...
.team-card:nth-child(n+7){animation-delay:.7s}
</style>
  ...
  <nav class="navbar">
    ...
    <div class="action-buttons">
      <div class="dropdown" id="loginDropdown">
        <button class="dropdown-toggle" type="button">Login <span class="arrow">▾</span></button>
        <ul class="dropdown-menu">
          <li><a href="https://nersapp.com/s/amfus/auth/">Student Portal</a></li>
          <li><a href="https://amfuscomprehensivemodelschool.com.ng/admin/admin_login.php">Admin Dashboard</a></li>
        </ul>
  ...
  <?php if (mysqli_num_rows($result) > 0): ?>
    <div class="team-grid">
      <?php while ($member = mysqli_fetch_assoc($result)):
        ...
        $defaultImage = 'https://via.placeholder.com/280x280/4169e1/ffffff?text=' . urlencode(substr($member['full_name'], 0, 1));
          ...
          <?php if (!empty($member['facebook_url']) || !empty($member['twitter_url']) || !empty($member['linkedin_url'])): ?>
            <div class="socials">
              <?php if (!empty($member['facebook_url'])): ?>
                <a href="<?php echo htmlspecialchars($member['facebook_url']); ?>" target="_blank" rel="noopener noreferrer" aria-label="Facebook">
                  <i class="fab fa-facebook-f"></i>
                </a>
              <?php endif; ?>

              <?php if (!empty($member['twitter_url'])): ?>
                <a href="<?php echo htmlspecialchars($member['twitter_url']); ?>" target="_blank" rel="noopener noreferrer" aria-label="Twitter">
                  <i class="fab fa-twitter"></i>
                </a>
              <?php endif; ?>

              <?php if (!empty($member['linkedin_url'])): ?>
                <a href="<?php echo htmlspecialchars($member['linkedin_url']); ?>" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">
