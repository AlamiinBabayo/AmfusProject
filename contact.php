...
<head>
  ...
  <meta name="description" content="Contact Amfus Comprehensive Model School">
...
body{font-family:system-ui,Arial,Helvetica,sans-serif;color:var(--text);background:#f4f6f8;}
...
/* Dropdown menu (About Us + Login on desktop) */
.dropdown-menu{
  display:none;
  position:absolute;
  top:100%;
...
.btn-apply:hover{filter:brightness(.95);transform:translateY(-1px)}

/* ===== DESKTOP LOGIN BUTTON (USES SAME DROPDOWN AS ABOUT) ===== */
.action-buttons .dropdown { position: relative; }

/* Make it look like a blue button, but still "dropdown-toggle" */
...
/* Align login menu to the right on desktop */
...
@media (max-width:920px){
  ...
  /* Make dropdown menus inside mobile menu look stacked */
  .dropdown-menu{
    position:static;
    box-shadow:none;
    border-radius:0;
  ...
  /* Hide right action buttons on mobile */
  ...
  /* ✅ Mobile Login as BUTTON + stacked menu */
  .mobile-login-wrap{
...
/* ✅ Hide the mobile login wrap on desktop (so web has only ONE login) */
@media (min-width:921px){
...
/* ===== CONTACT PAGE UI ===== */
h1, h2 { text-align: center; margin-bottom: 20px; color: #ff4b5c; }
...
.card i{ font-size:45px;color:#ff4b5c;margin-bottom:15px;font-style: italic; }
.card h3{ margin-bottom:10px;color:#4169e1;font-style: italic; }
.card p, .card a{ font-size:16px;line-height:1.5;color:#333;text-decoration:none;font-style: italic; }
.card.show{ animation: popUp 1s ease forwards; }
@keyframes popUp{ 0%{opacity:0; transform:translateY(60px);} 100%{opacity:1; transform:translateY(0);} }
...
.contact-form.show{ animation: slideInLeft 1s ease forwards; }
...
.contact-form button:hover{ background:#e63c50; transform:scale(1.05); }
...
.map-container.show{ animation: zoomIn 1s ease forwards; }
...
.cta-buttons:hover{ background: linear-gradient(135deg, #666465, #1a1a1a); transform: translateY(-1px); }

@media (max-width:900px){
  .cards{ flex-direction:column; align-items:center; }
  .card{ flex:1 1 100%; width:90%; }
}
...
.footer.show{ animation: slideUp 1.2s ease forwards; }
...
<header class="main-header" style="font-style: italic; font-weight: bold;">
  <nav class="navbar">
    ...
    <!-- ✅ DESKTOP RIGHT SIDE: ONLY ONE LOGIN + APPLY -->
...
(function(){
  ...
  const dropdowns = document.querySelectorAll('.nav-links .dropdown'); // About dropdown only
  ...
  // Desktop login behaves like About Us (hover on desktop).
  // On mobile, allow click to open (optional)
